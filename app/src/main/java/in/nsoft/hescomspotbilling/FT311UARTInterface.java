...
public class FT311UARTInterface
{
	...
	static String tempsread  = "";//17-05-2016
	...
	//17-05-2016
	public byte SendData(int numBytes, byte[] buffer,int cnt) 
	{
		status = 0x00; /*success by default*/
		/*
		 * if num bytes are more than maximum limit
	...
	//17-05-2016
	private void SendPacket(int numBytes,int count)
	{	
		try {
			if(outputstream != null){
				...
				{
					//c = 1;
					SetConfig(9600, (byte)7, (byte)1, (byte)2, (byte)0); //Changing Baud rate to 9600
				}
				else if(count == 8)
				{
					//d = 1;
	...
	public void DestroyAccessory(boolean bConfiged){
		...
		{
			SetConfig(4800,(byte)1,(byte)7,(byte)2,(byte)0);  // send default setting data for config
			try{Thread.sleep(10);}
			catch(Exception e){}
	...
	protected void saveDefaultPreference() {
		if(intsharePrefSettings != null)
		{
			...
			intsharePrefSettings.edit().putInt("baudRate", 4800).commit();
	...
	private final BroadcastReceiver mUsbReceiver = new BroadcastReceiver() 
	{
		...
		public void onReceive(Context context, Intent intent) 
		{
			...
			{
				Toast.makeText(global_context, " ACtion USB Permission", Toast.LENGTH_SHORT).show();
				synchronized (this)
				{
			...
			else if (UsbManager.ACTION_USB_ACCESSORY_DETACHED.equals(action)) 
			{
				...
				Intent i = new Intent(context , ExitUSBActivity.class);				
				i.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);			
				i.putExtra("Exit", intent.getStringExtra("Exit"));
				...
				try
				{
					...
					{	
						...
						if(readcount > 0)
						{
							...
							tempsread = new String(readBuffer,"UTF-8");//17-05-2016
